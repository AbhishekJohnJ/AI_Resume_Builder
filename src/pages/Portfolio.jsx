...
${markup}
  ...
  const getExportedCSS = () => {
    ...
    try {
      ...
      // Inject resolved theme color variables at the top if a theme was applied
      if (themeColor) {
        css += `/* Theme color overrides */\n`;
        css += `.pt${tplId} {\n`;
        css += `  --tc: ${themeColor.main};\n`;
        css += `  --tc-dark: ${themeColor.dark};\n`;
        css += `  --tc-light: ${themeColor.light};\n`;
        css += `  --tc-accent: ${themeColor.accent};\n`;
        css += `}\n\n`;
      }
